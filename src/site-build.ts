...
function renderHeader(
  ...
  const label = site.author ?? site.title;
  ...
  const socialLink = site.social
    ? `\n          <a href="${escapeHtml(site.social.url)}" rel="me">${escapeHtml(site.social.label)}</a>`
    : "";

  return `<header>
        ${brand}
        <nav aria-label="Site">
          ${aboutLink}${socialLink}
        </nav>
      </header>`;
}
...
function renderFooter(
  ...
  const socialLink = site.social
    ? ` <a href="${escapeHtml(site.social.url)}" rel="me">${escapeHtml(site.social.label)}</a>`
    : "";
  const sourceLink = site.sourceUrl
    ? `<a href="${escapeHtml(site.sourceUrl)}">Source</a>`
    : "";

  return `<footer>
        <span>${escapeHtml(author)}${socialLink}</span>
        ${generatedBy}
        ${sourceLink}
      </footer>`;
}
...
export function generateFaviconSvg(site?: SiteDefinition): string {
  const label = (site?.faviconText ?? "PF").slice(0, 2).toUpperCase();

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <rect width="64" height="64" rx="14" fill="#050505"/>
  <text x="32" y="40" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="23" font-weight="700" fill="#ffffff">${escapeHtml(label)}</text>
</svg>
`;
}
...
export function generateSitePage(
  ...
  const title =
    page.path === "/"
      ? site.title
      : `${page.title} - ${site.author ?? site.title}`;
...
export function generatePostPage(
  ...
  return renderLayout({
    site,
    path: postPath(post),
    title: `${post.title} - ${site.author ?? site.title}`,
    ...
    body: `        <h1 class="post-title">${escapeHtml(post.title)}</h1>
        <div class="meta">${site.social ? `<a href="${escapeHtml(site.social.url)}" rel="me">${escapeHtml(site.social.label)}</a> | ` : ""}<time datetime="${escapeHtml(post.publishedAt)}">${escapeHtml(formatDate(post.publishedAt))}</time><span data-relative-date-label data-published-at="${escapeHtml(post.publishedAt)}" hidden> | <span data-relative-date></span></span></div>
